...
void sixtop_init() {
    ...
    sixtop_vars.periodMaintenance  = 436 +(openrandom_get16b()&0xff); //Yadhu changed from 872 to 436.. To increase the beacon frequency.
...
void sixtop_request(
    ...
    //openserial_printf(pkt->l2_nextORpreviousHop.addr_64b,8,'D');
    len  = 0;
    if (
...
/**
 * @brief      { Injects payload directly to mac layer }
 *
 * @param      buffer_ptr  pointer buffer containing payload
 * @param[in]  len   Length of the payload
 *
 * @return     { returns whether packet was injected sucessfully or not }
 */

bool sixtop_packet_inject(uint8_t *buffer_ptr,uint8_t len)
{
    OpenQueueEntry_t* data;
    uint8_t i;
    uint8_t dagroot_addr[] = {0x14,0x15,0x92,0x00,0x00,0x00,0x00,0x01};

        // get a free packet buffer
    data = openqueue_getFreePacketBuffer(COMPONENT_SIXTOP);
        if (data==NULL) {
        openserial_printError(
            COMPONENT_SIXTOP,
            ERR_NO_FREE_PACKET_BUFFER,
    ...
    data->creator = COMPONENT_SIXTOP;
    data->owner   = COMPONENT_SIXTOP;
    packetfunctions_reserveHeaderSize(data,len);
    memcpy(data->payload,buffer_ptr,len);

    data->l2_frameType                     = IEEE154_TYPE_DATA;
    data->l2_nextORpreviousHop.type        = ADDR_64B;
    memcpy(&(data->l2_nextORpreviousHop.addr_64b[0]),dagroot_addr,8);
    sixtop_send_internal(data,FALSE);
...
port_INLINE void sixtop_sendEB() {
    ...
    //Yadu's modification, Only dagroot will send beacons.
    //For Others to synchronize.
    if(ieee154e_isSynch() == FALSE || !idmanager_getIsDAGroot())
    //if(ieee154e_isSynch() == FALSE)
        return;
