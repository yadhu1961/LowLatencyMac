...
port_INLINE uint8_t change_channel(cellType_t slot_type);
...
void ieee154e_startOfFrame(PORT_TIMER_WIDTH capturedTime) {
   //ticks_measure  = opentimers_measure_ticks(0,1);
   //openserial_printf(&ticks_measure,1,'D');
...
port_INLINE void activity_synchronize_newSlot() {
   ...
   if        ((ieee154e_vars.asn.bytes0and1&0x000f)==0x0000) {
      openserial_stop();
      openserial_startOutput(ieee154e_vars.asn.bytes0and1 & 0xff,ieee154e_vars.slotOffset);
   } else if ((ieee154e_vars.asn.bytes0and1&0x000f)==0x0008) {
      openserial_stop();
      openserial_startInput(ieee154e_vars.asn.bytes0and1 & 0xff,ieee154e_vars.slotOffset);
   }
}
...
port_INLINE void activity_ti1ORri1() {
   ...
   if (ieee154e_vars.slotOffset==ieee154e_vars.nextActiveSlotOffset) {
      ...
      // calculate the frequency to transmit on, Yadu disabled this.
      ...
      //openserial_printf(&ieee154e_vars.slotOffset,1,'D');
   ...
   ieee154e_vars.freq = change_channel(cellType);
   switch (cellType) {
      case CELLTYPE_TXRX:
      case CELLTYPE_TX:
         ...
         if (schedule_getOkToSend()) {
            ...
            //Yadhu added starts as part of static schedule
            memset(&neighbor,0,sizeof(neighbor));
            neighbor.type             = ADDR_ANYCAST;//Allows to send a packet to ANYONE
            if(cellType == CELLTYPE_TX) //This is to transmit data packet only in TX slot.
               //yadhu added ends,
         ...
         if (ieee154e_vars.dataToSend==NULL) {
            ...
            //This code is been added to plot distribution of queuing times.
            if(ieee154e_vars.dataToSend->l2_frameType == IEEE154_TYPE_DATA)
            {
               uint8_t temp[3] = {ieee154e_vars.asn.bytes0and1 & 0xff,CELLTYPE_TX,ieee154e_vars.slotOffset&0xff};
               // uint16_t ticks = opentimers_measure_ticks(0,1);
               // temp[0] = ticks & 0xff;
               // temp[1] = ticks >> 8;
               // temp[2] = ieee154e_vars.slotOffset;
               // printing slot offset of tx
      ...
      case CELLTYPE_RX:
         if (changeToRX==FALSE) {
            //This happens only in Rx slot, not in TXRX slot.
            // stop using serial
            openserial_stop();
         }
         // change state
...
port_INLINE void activity_ti2() {
    ...
    //openserial_printf(&(ieee154e_vars.localCopyForTransmission.length),1,'D');

    //openserial_printf(ieee154e_vars.localCopyForTransmission.payload,ieee154e_vars.localCopyForTransmission.length,'D');
    ...
    */
    do { // this "loop" is only executed once
        ...
        if (ieee802514_header.ackRequested==1 && ieee154e_vars.isAckEnabled == TRUE) {
            ...
            // Here I need to check whether beacon is from DAGroot, If so synchronize with it.
            if ((idmanager_getIsDAGroot()==FALSE))// &&
        ...
        if (subtype == 1) {
            ...
            case IEEE802154E_MLME_SLOTFRAME_LINK_IE_SUBID:
                ...
                if (oldFrameLength==0){
                  ...
                  //Yadhu added for static scheduling
...
void notif_receive(OpenQueueEntry_t* packetReceived) {
   //Added this avoid dependency of latency over scheduling of microcontroller begins
   if(packetReceived->l2_frameType == IEEE154_TYPE_DATA) {
      openserial_printf((uint8_t*)(packetReceived->payload),packetReceived->length,'P');
      uint8_t temp[3] = {ieee154e_vars.asn.bytes0and1 & 0xff,CELLTYPE_RX,ieee154e_vars.slotOffset&0xff};
      //openserial_printf(&temp,3,'D');
      openqueue_freePacketBuffer(packetReceived);
      return;
   }
   //Added this avoid dependency of latency over scheduling of microcontroller ends
   // record the current ASN
...
/**
 * { returns the channel number based on type of slot}
 */
port_INLINE uint8_t change_channel(cellType_t slot_type) {
